  ...
  switch (config.appOverview) {
    case "AppOverview.json": {
      swaps = swaps.concat([
        ...
        {
          from: "InternalCompareAppID",
          to: config.compareAppID == "" ? config.appID : config.compareAppID,
          wrap: true,
        },
        {
          from: "MyCompareApp",
          to: config.compareAppName == "None" ? config.appName : config.compareAppName,
          wrap: true,
        },
        {
          from: "-MyCompareTimeh to -MyTimeh",
          to: config.compareTime,
          wrap: true,
        },
...
function generateFunnelSwapList(config) {
  var swaps = [
    ...
    {
      from: "comparerevenueproperty",
      to: typeof config.compareRevenue == "undefined" ? config.kpi : config.compareRevenue,
      wrap: true,
    },
  ...
  for (let i = funnelSteps.length - 1; i >= 0; i--) {
    //go in reverse because steps are not zero padded
    let j = i + 1;
    swaps.push({
      from: "StepHeader" + j,
      to: config.funnelData[i].label,
      wrap: true,
    });
  }
  ...
  if (config.featureAdded) {
    swaps.push({
      from: "FeatureHeaderStep",
      to: config.FeatureHeaderStep,
      wrap: true,
    });
  } else {
...
function whereClauseSwaps(dbData, config) {
  ...
  if (config.campaignActive) {
    //special handling for Marketing campaign: insert into all Step1s but replace completely on Marketing pages
    if (dbData["dashboardMetadata"]["name"].includes("Marketing"))
      whereSteps[0] = "(" + config.campaignStep1.colname + '="' + config.campaignStep1.name + '") ';
    else
      whereSteps[0] = whereSteps[0].replace(
        /\) ?$/,
        " OR " + config.campaignStep1.colname + '="' + config.campaignStep1.name + '") '
      );
  }
  ...
  if (config.featureAdded)
    whereSwaps.push({
      from: "StepNewFeature1",
      to: config.StepNewFeature1.colname + '="' + config.StepNewFeature1.name + '"',
      wrap: true,
    });
  if ("compareAppName" in config && config.compareAppName != "None") {
    whereSwaps.push({
      from: "CompareStepFunnel1",
      to: config.compareFirstStep.colname + '="' + config.compareFirstStep.name + '"',
      wrap: true,
    }); //V5
    whereSwaps.push({
      from: "CompareStepAction1",
      to: config.compareFirstStep.name,
      wrap: true,
    }); //V4
    whereSwaps.push({
      from: "CompareLastFunnelStep",
      to: config.compareLastStep.colname + '="' + config.compareLastStep.name + '"',
      wrap: true,
    }); //V5
    whereSwaps.push({
      from: "CompareLastStep",
      to: config.compareLastStep.name,
      wrap: true,
    }); //V4
    whereSwaps.push({
      from: "CompareCombinedStep",
      to:
        "(" +
        config.compareFirstStep.colname +
        '="' +
        config.compareFirstStep.name +
        '" AND ' +
        config.compareLastStep.colname +
        '="' +
        config.compareLastStep.name +
        '") ',
      wrap: true,
    });
    ...
    //no compare app, default stuff out per Shady
    whereSwaps.push({
      from: "CompareStepFunnel1",
      to: whereSteps[0],
      wrap: true,
    ...
    whereSwaps.push({
      from: "CompareLastFunnelStep",
      to: whereSteps[whereSteps.length - 1],
      wrap: true,
    });
    whereSwaps.push({
      from: "CompareLastStep",
  ...
  for (let i = whereSteps.length - 1; i >= 0; i--) {
    ...
    if (i == whereSteps.length - 1) {
      ...
      whereSwaps.push({
        from: 'useraction.name ?= ?"LastStep"',
        to: whereSteps[i],
        wrap: true,
      });
      whereSwaps.push({
        from: 'useraction.name ?[iInN]{2} ?\\("LastStep"\\)',
        to: whereSteps[i],
        wrap: true,
      });
      whereSwaps.push({
...
function doSwaps(db, swaps) {
  ...
  if (typeof db == "string") {
    //already a string, great do the swaps
    dbS = db;
  } else if (typeof db == "object") {
    //handles url encoded tiles, then stringify, replace again
...
function transformSubs(subs, dbid, swaps, config, nextID = nextDB) {
  ...
  subs.forEach(function (db) {
    ...
    if (config.addBackButtons !== false) addReplaceButton(sub, dbid, "![BackButton]()", "⇦", findTopRight);
  });
...
function generateWorkflowSwapList(el, customSwaps = null) {
  ...
  $el.find(".workflowInput").each(function (i, el) {
    ...
    let whereClause = $slicer.attr("data-addWhereClause") === "true" ? true : false;
    ...
    if (journeyPicker.length) {
      ...
      switch (slicer) {
        ...
        default: {
          ...
          if (!whereClause) {
            //added to maintain logic when moving else if(whereClause)
            let from = "${" + transform + "}";
...
function queryDoSwaps(query, swaps) {
  swaps.forEach(function (swap) {
    ...
    if (query.match(regex) && swap.to.substring(0, 1) == '"' && swap.to.substr(-1) == '"') {
      //handle quoted string, which is about to be inserted into a quoted string
      ...
      if (query.match(regex2))
        //unwrap double-wrapped quotes
...
function addTileReplication(workflowInput, swaps) {
  ...
  if (replicationPriorTransform.substr(0, 2) == "${" && replicationPriorTransform.substr(-1) == "}")
...
function usqlSelectGetSwaps(slicer, workflowInput, transform, swaps, whereClause) {
  ...
  switch (slicer) {
    ...
    case "Keys/Values_edit": {
      ...
      let fromkey = `\${${transform}.key}`;
      let fromname = `\${${transform}.name}`;
      let fromnameenc = `\${${transform}.nameenc}`;
      let fromval = `\${${transform}.value}`;
      let fromvalenc = `\${${transform}.valueenc}`;
      addToSwaps(swaps, { from: fromkey, to: key });
      addToSwaps(swaps, { from: fromname, to: name });
      addToSwaps(swaps, { from: fromval, to: val });
      addToSwaps(swaps, { from: fromnameenc, to: encodeURIComponent(name) });
    ...
    case "Keys/Values": {
      ...
      if ($val.length) {
        ...
        if (multi) {
          ...
          let fromvalenc = `\${${transform}.valueenc}`;
          addToSwaps(swaps, { from: fromval, to: value });
          addToSwaps(swaps, { from: fromvalenc, to: encodeURIComponent(value) });
        }
      }
      break;
    }
    ...
    case "actions": {
      ...
      let nameenc = `\${${transform}.nameenc}`;
      if ($option.length === 1) {
        addToSwaps(swaps, { from: from, to: value });
        addToSwaps(swaps, { from: raw, to: value });
        addToSwaps(swaps, { from: nameenc, to: encodeURIComponent(value) });
