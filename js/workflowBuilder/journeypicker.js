...
function JourneyPickerFactory(target, app, data = null) { //JourneyPicker factory, usage: var jp = JourneyPickerFactory("#viewport",{name:"www.angular.easytravel.com",id:"APPLICATION-726A108B51CB78E2"});
	...
	function populateMethodList() {
		...
		let params = `pageSize=5000&metricSelector=builtin:apps.web.action.apdex:merge("User type")&resolution=Inf&from=now-1d/d&entitySelector=type(APPLICATION_METHOD),fromRelationships.isApplicationMethodOf(entityId(${app.ids.join(',')}))`;
		let query = "/api/v2/metrics/query?"+encodeURI(params);
		
		let p0 = dtAPIquery(query);
		promises.push(p0);
		//get all "important" actions
		params = `pageSize=5000&metricSelector=builtin:apps.web.action.duration.(xhr,load,custom).browser:(count):parents:names:merge("dt.entity.browser")&resolution=Inf&entitySelector=type(APPLICATION_METHOD),fromRelationships.isApplicationMethodOf(entityId(${app.ids.join(',')}))`;
		query = "/api/v2/metrics/query?"+encodeURI(params);
